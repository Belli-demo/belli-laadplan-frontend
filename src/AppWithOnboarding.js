...
function wijkFillColor(lp) {
  if (lp === 0)  return '#9EC5CB'; // lichtblauw = geen extra nodig
  if (lp <= 5)   return '#D4E8A0'; // lichtgroen = weinig nodig
  if (lp <= 15)  return '#F5D76E'; // geel = beperkt aantal nodig
  if (lp <= 30)  return '#5DBB7A'; // groen = extra nodig
  return '#E8683A';                // rood = veel nodig
}
...
export default function AppWithOnboarding() {
  ...
  useEffect(() => {
    ...
    existingPalen.forEach(el => {
      ...
      L.marker([lat,lng],{icon:makeBestaandIcon(paalType), zIndexOffset: 1000})
              ...
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                <thead>
                  <tr style={{background:'#0a1620',position:'sticky',top:0}}>
                    {['Wijk/Sector','Bij te plaatsen LP','Energievraag MWh/jr','CAPEX indicatief','Prioriteit'].map(h=>(
                ...
                <tbody>
                  {(() => {
                    const totaalLP  = wijkResults.reduce((s,r)=>s+r.data.totLP,0);
                    const totaalMwh = wijkResults.reduce((s,r)=>s+r.data.totMwh,0);
                    const rawGew = (sectoren?.features||[]).map(f=>parseFloat(f.properties?.GEWICHT)||1);
                    const somGew = rawGew.reduce((s,g)=>s+g,0);
                    const normGew = rawGew.map(g=>g/somGew);
                    const rijen = sectoren?.features?.length > 0
                      ? sectoren.features.map((f,i)=>({
                          naam: f.properties?.NAAM||'Sector '+(i+1),
                          lp:   Math.round(totaalLP  * normGew[i]),
                          mwh:  Math.round(totaalMwh * normGew[i]),
                          capex: Math.round(totaalLP * normGew[i]) * 4000,
                        }))
                      : wijkResults.map(({wijk,data})=>({
                          naam: wijk.naam,
                          lp:   data.totLP,
      ...
      <div style={st.mArea}>
        ...
        <div style={st.legB}>
          {[
            ...
            [{width:14,height:10,borderRadius:2,background:'#9EC5CB'},'Geen extra LP nodig'],
            [{width:14,height:10,borderRadius:2,background:'#F5D76E'},'Beperkt extra LP nodig'],
            [{width:14,height:10,borderRadius:2,background:'#5DBB7A'},'Extra LP nodig'],
            [{width:14,height:10,borderRadius:2,background:'#E8683A'},'Veel extra LP nodig'],
          ].map(([ds,l],i)=>(
