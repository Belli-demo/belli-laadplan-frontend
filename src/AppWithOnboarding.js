...
export default function AppWithOnboarding() {
  ...
  useEffect(() => {
    ...
    // Geen geprojecteerde palen op kaart
    setOngematcht(0);
  ...
  useEffect(() => {
    ...
    existingPalen.forEach(el => {
      ...
      if(!lat) return;
      const t=el.tags||{};
      // Bepaal type op basis van socket-tags en vermogen
      ...
      // Socket type detectie
      const heeftCCS  = t['socket:type2_combo'] || t['socket:ccs'] || t['socket:type2_combo:output'];
      const heeftCHAdeMO = t['socket:chademo'] || t['socket:chademo:output'];
      const heeftType2 = t['socket:type2'] || t['socket:type2:output'];
      const networkHPC = (t['network']||'').match(/ionity|fastned|tesla|allego|porsche/i);
      
      let paalType;
      if (maxPower >= 100 || networkHPC) {
        paalType = 'hpc';
      } else if (maxPower >= 22 || heeftCCS || heeftCHAdeMO) {
        paalType = 'dc';
      } else {
        paalType = 'ac';
      }
      ...
      <div style={st.mArea}>
        ...
        <div style={st.legB}>
          {[
            ...
            [{width:14,height:10,borderRadius:2,background:'#E8683A'},'Veel laadpunten nodig'],
          ].map(([ds,l],i)=>(
