...
export default function AppWithOnboarding() {
  ...
  useEffect(() => {
    ...
    // Gebruik gemeentecentrum als basis ipv wijkcoördinaten
    ...
    wijkResults.forEach(({ wijk, data }, wijkIdx) => {
      ...
      if (!baseLat || !baseLng) {
        if (wijk.lat && wijk.lng) {
          ...
          setOngematcht(prev => prev + totLPWijk);
        } else {
          const totLPWijk = Object.values(data.breakdown).reduce((s,b)=>s+b.ac+b.acs+b.dc+b.dcu,0);
          setOngematcht(prev => prev + totLPWijk);
          return;
  ...
  useEffect(() => {
    ...
    existingPalen.forEach(el => {
      ...
      if(!lat) return;
      const t=el.tags||{};
      L.marker([lat,lng],{icon:makeIcon('#4CAF50',10)})
       .bindPopup(`<b>Bestaande paal</b><br>${t.name||t.operator||'–'}<br>Netwerk: ${t.network||'–'}<br><small>Bron: OpenStreetMap</small>`)
       .addTo(g);
    });
  ...
  return (
    <div style={st.app}>
      ...
      <div style={st.mArea}>
        ...
        <div style={st.legB}>
          {[
            [{width:10,height:10,borderRadius:'50%',background:'#4CAF5044',border:'2px solid #4CAF50'},'Bestaande LP (OSM)'],
