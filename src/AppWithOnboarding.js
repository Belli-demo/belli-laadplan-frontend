...
export default function AppWithOnboarding() {
  ...
  useEffect(() => {
    ...
    // Geprojecteerde palen (altijd tonen)
    wijkResults.forEach(({ wijk, data }) => {
      let pIdx=0;
      const addP = (cnt, col, pop) => {
        for(let i=0;i<Math.min(cnt,6);i++){
          const a=(pIdx*137.5)*Math.PI/180, r=0.004+(pIdx%3)*0.002;
          L.marker([wijk.lat+r*Math.cos(a), wijk.lng+r*Math.sin(a)/Math.cos(wijk.lat*Math.PI/180)],
            {icon:makeIcon(col,12)}).bindPopup(pop).addTo(pg);
          pIdx++;
        }
      };
