...
export default function AppWithOnboarding() {
  ...
  useEffect(() => {
    ...
    if (mapInstance.current && gemeente) {
      ...
      console.log('Gemeente geswitcht naar:', gemId);
    }
  }, [gemId]);
  ...
  useEffect(() => {
    ...
    setOngematcht(0);
  }, [gemId, scenario, year, pubPct, activeSegs, trendFactor, gemeenten, sectorenGeladen]);
  ...
  useEffect(() => {
    ...
    existingPalen.forEach(el => {
      ...
      let paalType;
      if (maxPower >= 100 || networkHPC) paalType = 'hpc';
      else if (maxPower >= 22 || heeftCCS || heeftCHAdeMO) paalType = 'dc';
      else paalType = 'ac';
              ...
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                ...
                <tbody>
                  {(() => {
                    const totaalLP  = wijkResults.reduce((s,r)=>s+r.data.totLP,0);
                    const totaalMwh = wijkResults.reduce((s,r)=>s+r.data.totMwh,0);
                    const rawGew = (sectorenRef.current?.features||[]).map(f=>parseFloat(f.properties?.GEWICHT)||1);
                    const somGew = rawGew.reduce((s,g)=>s+g,0);
                    const normGew = rawGew.map(g=>g/somGew);
                    const rijen = sectorenRef.current?.features?.length > 0
