...
export default function AppWithOnboarding() {
  ...
  const [sectorenTeller,  setSectorenTeller] = useState(0);
  ...
  const loadSectoren = useCallback(async (id) => {
    try {
      ...
      const nieuweData = fc?.features?.length > 0 ? fc : null;
      setSectoren(nieuweData);
      setSectorenTeller(t => t + 1);
    } catch(e) {
      setSectoren(null);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    setOngematcht(0);
  }, [gemId, scenario, year, pubPct, activeSegs, trendFactor, gemeenten, sectoren, sectorenTeller]);
