...
export default function AppWithOnboarding() {
  ...
  // op de backend, hier client-side omdat existingPalen live via OSM
  // wordt opgehaald en niet in de database staat)
  const bestaandPerWijk = {};
  (gemeente?.wijken || []).forEach(w => { bestaandPerWijk[w.id] = 0; });
  existingPalen.forEach(el => {
    ...
    if (beste) bestaandPerWijk[beste.id] = (bestaandPerWijk[beste.id] || 0) + 1;
  });

  // ── Bereken wijken ─────────────────────────────────────────────────
  const wijkResults = (gemeente?.wijken || []).map(w => {
    const data = calcWijk(w, calcParams);
    const bestaand = bestaandPerWijk[w.id] || 0;
    const delta = Math.max(0, data.totLP - bestaand);
    return { wijk:w, data:{ ...data, bestaand, delta } };
  });
  const alleDeltas = wijkResults.map(r => r.data.delta);
  ...
  // Bijkomend = nodig - echt aanwezig (per wijk, al verrekend in .delta) - gepland
  const totaalDeltaNaBestaand = wijkResults.reduce((s,r)=>s+r.data.delta,0);
  const bijkomend = Math.max(0, totaalDeltaNaBestaand - huidigLP);
  // CAPEX-tegel: bruto bouwkost per wijk naar verhouding verminderd met wat al
  // aanwezig is (delta/totLP als grove schatting, geen exacte kost per
  // laadpaaltype omdat we niet weten welk type paal al bestaat)
  const capexBijkomend = wijkResults.reduce((s,r) => {
    const ratio = r.data.totLP > 0 ? r.data.delta / r.data.totLP : 0;
    return s + r.data.totCapex * ratio;
  }, 0);
  ...
  const tijdreeks = YEARS.map(yr => {
    ...
    const res = (gemeente?.wijken||[]).map(w => {
      ...
      const bestaand = bestaandPerWijk[w.id] || 0;
      return { ...d, delta: Math.max(0, d.totLP - bestaand) };
    });
    const capexNu = res.reduce((s,r) => {
      const ratio = r.totLP > 0 ? r.delta / r.totLP : 0;
      return s + r.totCapex * ratio;
    }, 0);
    return {
      jaar:yr,
      'Laadpunten nodig':  res.reduce((s,r)=>s+r.totLP,0),
      'Bijkomend': Math.max(0, res.reduce((s,r)=>s+r.delta,0) - huidigLP),
  ...
  useEffect(() => {
    ...
    if (sectorenRef.current?.features?.length > 0) {
      ...
      sectorenRef.current.features.forEach((feature, idx) => {
        ...
        const fillColor = match ? kwantielKleur(match.data.delta, alleDeltas) : '#5a6a72';
        const tooltip   = match
          ? `<b>${match.wijk.naam}</b><br>${match.data.totLP} nodig (bruto) · ${match.data.bestaand} aanwezig · <b>${match.data.delta} bijkomend</b><br>${Math.round(mwh)} MWh/jr`
    ...
    wijkResults.forEach(({ wijk, data }) => {
      ...
      const fc = kwantielKleur(data.delta, alleDeltas);
      L.circle([wijk.lat, wijk.lng], {
        radius:300+data.delta*18, color:fc, weight:2, fillColor:fc, fillOpacity:0.22,
      })
      .bindTooltip(`<b>${wijk.naam}</b><br>${data.totLP} nodig (bruto) · ${data.bestaand} aanwezig · <b>${data.delta} bijkomend</b><br>${Math.round(data.totMwh)} MWh/jr`)
  ...
  }, [gemId, scenario, year, pubPct, activeSegs, trendFactor, gemeenten, sectorenGeladen, existingPalen]);
        ...
        {selectedResult && (
          <div style={st.ov}>
            ...
            <div style={{fontSize:10,color:C.textDim,marginBottom:10}}>
              {selectedResult.data.delta>25?'🔴 HOOG':selectedResult.data.delta>15?'🟡 MIDDEL':'🟢 LAAG'} · {year}
            </div>
            {[
              ['Laadpunten nodig (bruto)', selectedResult.data.totLP],
              ['Al aanwezig',              selectedResult.data.bestaand],
              ['Nog te plaatsen',          selectedResult.data.delta],
