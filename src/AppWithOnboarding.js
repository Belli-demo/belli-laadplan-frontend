...
export default function AppWithOnboarding() {
  ...
  useEffect(() => {
    ...
    if (sectoren?.features?.length > 0) {
      // CHOROPLETH: officiele statistische sectoren
      sectoren.features.forEach(feature => {
        const naam = feature.properties?.NAAM || 'Onbekend';
        // Match sector aan wijk via naam (fuzzy)
        const matchedWijk = gemeente.wijken.find(w =>
          naam.toLowerCase().includes(w.naam.toLowerCase().split(' ')[0].toLowerCase()) ||
          w.naam.toLowerCase().includes(naam.toLowerCase().split(' ')[0].toLowerCase())
        ) || gemeente.wijken[0];

        const wd = wijkDataMap[matchedWijk?.id];
        const lp = wd?.data?.totLP || 0;
        const mwh = wd?.data?.totMwh || 0;
        ...
        try {
          ...
          poly.bindTooltip(`<b>${naam}</b><br>${lp} LP nodig · ${Math.round(mwh)} MWh/jr`,
            { sticky: true });
          ...
          console.warn('Polygon render fout:', naam, e.message);
        }
      });
    } else {
