...
import { getAlleGemeenten, getGemeente, slaGemeenteOp, verwijderGemeente, checkHealth, onboardGemeenteGeo, getSectoren } from './api';
...
export default function AppWithOnboarding() {
  ...
  const tijdreeks = YEARS.map(yr => {
    ...
    return {
      jaar:yr,
      'LP nodig':  res.reduce((s,r)=>s+r.totLP,0),
  ...
  const voegGemeenteToe = useCallback(async (nieuw) => {
    ...
    try {
      ...
      // Start geo onboarding op de achtergrond
      try { await onboardGemeenteGeo(opgeslagen?.id || nieuw.id); } catch(e) { console.warn('Geo onboard:', e.message); }
    } catch(e) {
    ...
    if (sectoren?.features?.length > 0) {
      ...
      sectoren.features.forEach((feature, idx) => {
        ...
        try {
          ...
          poly.bindTooltip(
            `<b>${naam}</b><br>${lp} LP nodig · ${Math.round(mwh)} MWh/jr`,
            { sticky: true }
          );
          poly.on('click', () => {
        ...
        <div style={st.scroll}>
          ...
          <div style={st.sec}>
            ...
            <div style={st.sBody}>
              ...
              <div>
                <div style={st.lbl}><span>Huidig # LP</span><span style={st.lv}>{huidigLP}</span></div>
          ...
          <div style={st.sec}>
            ...
            <div style={st.sBody}>
              <div style={st.stG}>
                {[['LP nodig',fmtN(totLP),C.teal,'publiek domein'],
          ...
          <div style={{fontSize:11,color:C.textDim}}>
            ...
            {ongematcht > 0 && (
              <span style={{marginLeft:12, color:C.gold, fontSize:11}}>
                ⚠ {ongematcht} LP zonder wijkkoppeling
              </span>
            )}
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height={128}>
            {chartTab==='mwh'?(
              ...
              <LineChart data={tijdreeks} margin={{top:0,right:8,left:-24,bottom:0}}>
                ...
                <YAxis tick={{fill:C.textDim,fontSize:10}}/>
                <Tooltip contentStyle={{background:C.panelBg,border:`1px solid ${C.border}`,fontSize:11}}/>
                <Line dataKey="LP nodig" stroke={C.teal} strokeWidth={2} dot={false}/>
