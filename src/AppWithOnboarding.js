...
import { GEMEENTEN as FALLBACK_GEMEENTEN, SEG_PARAMS, calcWijk, YEARS, SCENARIOS } from './gemeenteData';
import GemeenteOnboarding from './components/GemeenteOnboarding';
import GemeenteEditor from './components/GemeenteEditor';
import { getAlleGemeenten, getGemeente, slaGemeenteOp, verwijderGemeente, checkHealth, onboardGemeenteGeo, getSectoren, syncWijkenVanStatbel } from './api';
...
function makeIcon(color, size=10) {
  return L.divIcon({
    className:'',
    html:`<div style="width:${size}px;height:${size}px;border-radius:50%;background:${color}44;border:2px solid ${color};box-shadow:0 0 6px ${color}88"></div>`,
    iconSize:[size,size], iconAnchor:[size/2,size/2],
  });
}

function makeBestaandIcon(type) {
  // Bestaande palen: volle, goed zichtbare markers
  const configs = {
    ac:   { color:'#1a8a3a', bg:'#2ecc71', label:'AC' },
    dc:   { color:'#c0392b', bg:'#e74c3c', label:'DC' },
    hpc:  { color:'#8e44ad', bg:'#9b59b6', label:'HPC' },
    default: { color:'#1a5276', bg:'#2980b9', label:'LP' },
  };
  ...
  return L.divIcon({
    ...
    html:`<div style="
      ...
      box-shadow:0 0 0 1.5px white, 0 2px 4px rgba(0,0,0,0.4);
    "></div>`,
...
export default function AppWithOnboarding() {
  ...
  const [sectoren,        setSectoren]       = useState(null);
  const [sectorenLayer,   setSectorenLayer]  = useState(null);
  ...
  const tijdreeks = YEARS.map(yr => {
    const p = {...calcParams, year:yr};
    const res = (gemeente?.wijken||[]).map(w => calcWijk(w,p));
    return {
      jaar:yr,
      'Laadpunten nodig':  res.reduce((s,r)=>s+r.totLP,0),
      'Bijkomend': Math.max(0, res.reduce((s,r)=>s+r.totLP,0) - huidigLP),
      'MWh':       Math.round(res.reduce((s,r)=>s+r.totMwh,0)),
      'CAPEX €K':  Math.round(res.reduce((s,r)=>s+r.totCapex,0)/1000),
    };
  });
  ...
  const loadSectoren = useCallback(async (id) => {
    try {
      ...
      setSectoren(fc?.features?.length > 0 ? fc : null);
    } catch(e) {
      setSectoren(null);
    }
  }, []);
  ...
  const voegGemeenteToe = useCallback(async (nieuw) => {
    ...
    try {
      ...
      // Geo onboarding + wijken sync op achtergrond
      const gId = opgeslagen?.id || nieuw.id;
      try {
        await onboardGemeenteGeo(gId);
        const syncResult = await syncWijkenVanStatbel(gId);
        if (syncResult.ok) {
          // Herlaad gemeente met nieuwe wijken
          const bijgewerkt = await getGemeente(gId);
          setGemeenten(prev => ({ ...prev, [gId]: bijgewerkt }));
        }
  ...
  const loadBestaandePalen = useCallback(async () => {
    if (!gemeente?.bbox) return;
    setLoadingPalen(true);
    const [s,w,n,e] = gemeente.bbox;
    const q = `[out:json][timeout:25];(node["amenity"="charging_station"](${s},${w},${n},${e});way["amenity"="charging_station"](${s},${w},${n},${e}););out center;`;
    try {
      const r = await fetch('https://overpass-api.de/api/interpreter',
        { method:'POST', body:'data='+encodeURIComponent(q) });
      const json = await r.json();
      setExistingPalen(json.elements || []);
    } catch { setExistingPalen([]); }
  ...
  useEffect(() => {
    ...
    if (sectoren?.features?.length > 0) {
      // CHOROPLETH: officiele wijkpolygonen
      // Verdeel LP gelijkmatig over sectoren als geen directe match
      const totaalLP = wijkResults.reduce((s,r) => s + r.data.totLP, 0);
      const totaalMwh = wijkResults.reduce((s,r) => s + r.data.totMwh, 0);
      const nSectoren = sectoren.features.length;

      sectoren.features.forEach((feature, idx) => {
        const naam = feature.properties?.NAAM || `Sector ${idx+1}`;

        // Probeer wijk te matchen, anders verdeel evenredig
        const matchedWijk = gemeente.wijken.find(w => {
          const wn = w.naam.toLowerCase();
          const fn = naam.toLowerCase();
          return fn.includes(wn.split(' ')[0]) || wn.includes(fn.split(' ')[0]);
        });

        const wd = matchedWijk ? wijkDataMap[matchedWijk.id] : null;
        const lp  = wd ? wd.data.totLP  : Math.round(totaalLP  / nSectoren);
        const mwh = wd ? wd.data.totMwh : Math.round(totaalMwh / nSectoren);
        const fillColor = wijkFillColor(lp);
        ...
        try {
          ...
          poly.bindTooltip(
            `<b>${naam}</b><br>${lp} Laadpunten nodig · ${Math.round(mwh)} MWh/jr`,
            { sticky: true }
          );
          poly.on('click', () => {
            if (matchedWijk) setSelectedWijk(matchedWijk.id);
          });
          poly.addTo(wg);
        } catch(e) {
      ...
      // FALLBACK: cirkels als geen polygonen beschikbaar
      wijkResults.forEach(({ wijk, data }) => {
        const fc = wijkFillColor(data.totLP);
        L.circle([wijk.lat, wijk.lng], {
          radius:300+data.totLP*18, color:fc, weight:2, fillColor:fc, fillOpacity:0.22,
        })
        .bindTooltip(`<b>${wijk.naam}</b><br>${data.totLP} LP · ${Math.round(data.totMwh)} MWh/jr`)
    ...
    // Geprojecteerde palen — verspreid binnen de gemeente bbox
    const gemCenterLat = gemeente.center[0];
    const gemCenterLng = gemeente.center[1];
    let ongematchTeller = 0;
    
    wijkResults.forEach(({ wijk, data }, wijkIdx) => {
      // Probeer centroïd van sector te vinden als die beschikbaar is
      // Exacte match: wijknaam IS de Statbel sectornaam (na sync)
      const sectorFeature = sectoren?.features?.find(f => {
        const fn = (f.properties?.NAAM||'').toLowerCase();
        const wn = wijk.naam.toLowerCase();
        return fn === wn || fn.includes(wn) || wn.includes(fn);
      });

      let baseLat, baseLng;
      if (sectorFeature?.geometry) {
        // Bereken centroïd van de eerste polygon ring
        const coords = sectorFeature.geometry.type === 'MultiPolygon'
          ? sectorFeature.geometry.coordinates[0][0]
          : sectorFeature.geometry.coordinates[0];
        if (coords && coords.length > 0) {
          baseLng = coords.reduce((s,c) => s+c[0], 0) / coords.length;
          baseLat = coords.reduce((s,c) => s+c[1], 0) / coords.length;
        }
      }
      
      // Geen polygoon match gevonden
      if (!baseLat || !baseLng) {
        if (wijk.lat && wijk.lng) {
          // Gebruik wijkcoördinaat maar tel als ongematcht
          baseLat = wijk.lat;
          baseLng = wijk.lng;
          const totLPWijk = Object.values(data.breakdown).reduce((s,b)=>s+b.ac+b.acs+b.dc+b.dcu,0);
          ongematchTeller += totLPWijk;
        } else {
          const totLPWijk = Object.values(data.breakdown).reduce((s,b)=>s+b.ac+b.acs+b.dc+b.dcu,0);
          ongematchTeller += totLPWijk;
          return;
      ...
      let pIdx = wijkIdx * 20;
      const addP = (cnt, col, pop) => {
        for(let i=0;i<Math.min(cnt,6);i++){
          const a=(pIdx*137.5)*Math.PI/180;
          const r=0.002+(pIdx%4)*0.001;
          L.marker(
            [baseLat+r*Math.cos(a), baseLng+r*Math.sin(a)/Math.cos(baseLat*Math.PI/180)],
            {icon:makeIcon(col,12)}
          ).bindPopup(pop).addTo(pg);
          pIdx++;
        }
      };
      ...
      const totOV=data.breakdown.ov?.dcu||0;
      const pop=(t,v)=>`<b>${wijk.naam}</b><br>${t}<br>Vermogen: ${v}<br>Jaar: ${year}<br>OCPP 2.0.1 · Slim laden verplicht`;
      if(totAC>0) addP(totAC,'#9EC5CB',pop('AC Normaal/Snel','7–22 kW'));
      if(totDC>0) addP(totDC,'#D0AC41',pop('DC Snel','50–150 kW'));
      if(totOV>0) addP(totOV,'#c89ecb',pop('DC Ultrafast / OV Hub','>150 kW'));
    });

    wg.addTo(mapInstance.current); wijkLayerRef.current = wg;
    pg.addTo(mapInstance.current); projLayerRef.current = pg;
    setOngematcht(ongematchTeller);
  }, [gemId, scenario, year, pubPct, activeSegs, trendFactor, gemeenten, sectoren]);
  ...
  useEffect(() => {
    ...
    existingPalen.forEach(el => {
      ...
      if(!lat) return;
      const t=el.tags||{};
      // Bepaal type op basis van tags
      const socket = t['socket:type2_combo'] || t['socket:chademo'] || t['socket:type2'] || '';
      const maxPower = parseInt(t['maxpower'] || t['capacity:output'] || '0');
      const paalType = maxPower >= 100 ? 'hpc' : maxPower >= 22 ? 'dc' : 'ac';
      
      L.marker([lat,lng],{icon:makeBestaandIcon(paalType)})
       .bindPopup(`<b>Bestaande laadpaal</b><br>
         Type: ${paalType.toUpperCase()}<br>
         ${t.name||t.operator||'–'}<br>
         Netwerk: ${t.network||t.operator||'–'}<br>
         Vermogen: ${maxPower ? maxPower+'kW' : 'onbekend'}<br>
         <small>Bron: OpenStreetMap</small>`)
       .addTo(g);
    });
  ...
  return (
    <div style={st.app}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}} ::-webkit-scrollbar{width:4px} ::-webkit-scrollbar-thumb{background:#1e3a46;border-radius:2px}`}</style>
      ...
      <div style={st.side}>
        <div style={st.logo}>
          <div>
            <span style={{ fontSize:17, fontWeight:800, color:C.teal }}>belli</span>
        ...
        <div style={st.scroll}>
          ...
          {/* Scenario & jaar */}
          <div style={st.sec}>
            <div style={st.sHdr}>Scenario & jaar</div>
            <div style={st.sBody}>
              <div style={st.lbl}>
                <span>EV-scenario</span>
                <span style={st.lv}>{SCENARIOS.find(s=>s.id===scenario)?.naam}</span>
              </div>
              <input type="range" style={st.sl} min={0} max={2} step={1}
          ...
          <div style={st.sec}>
            ...
            <div style={st.sBody}>
              <div style={{ marginBottom:10 }}>
                <div style={st.lbl}><span>% publiek laden</span><span style={st.lv}>{pubPct}%</span></div>
                <input type="range" style={st.sl} min={10} max={65} step={1} value={pubPct} onChange={e=>setPubPct(+e.target.value)}/>
              </div>
              <div>
                <div style={st.lbl}><span>Huidig # laadpunten</span><span style={st.lv}>{huidigLP}</span></div>
                <input type="range" style={st.sl} min={0} max={500} step={10} value={huidigLP} onChange={e=>setHuidigLP(+e.target.value)}/>
              </div>
            </div>
          </div>
          ...
          {/* Segmenten */}
          <div style={st.sec}>
            <div style={st.sHdr}>Doelgroepen</div>
            <div style={st.sBody}>
              <div style={st.segG}>
                {Object.entries(SEG_PARAMS).map(([sid,p])=>(
                  <div key={sid} style={st.segB(activeSegs[sid],p.color)}
                    onClick={()=>setActiveSegs(s=>({...s,[sid]:!s[sid]}))}>
                    {p.label}
                  </div>
                ))}
              </div>
            ...
            <div style={st.sBody}>
              <div style={st.stG}>
                {[['Laadpunten nodig',fmtN(totLP),C.teal,'publiek domein'],
                  ['Bijkomend',fmtN(bijkomend),C.gold,'t.o.v. huidig'],
                  ['MWh/jaar',fmtN(totMwh),C.teal,'publieke vraag'],
                  ['CAPEX',fmtEur(totCapex),C.warn,'indicatief']
                ].map(([l,v,c,sub])=>(
      ...
      <div style={st.mArea}>
        <div style={st.mHdr}>
          ...
          <div style={{fontSize:11,color:C.textDim}}>
            ...
            {loadingPalen?'Laden…':`${existingPalen.length} bestaande LP (OSM)`}
            &nbsp;·&nbsp;
            <strong style={{color:C.teal}}>{SCENARIOS.find(s=>s.id===scenario)?.naam}</strong>
            {ongematcht > 0 && (
        ...
        {selectedResult && (
          <div style={st.ov}>
            ...
            <div style={{fontSize:10,color:C.textDim,marginBottom:10}}>
              {selectedResult.data.totLP>25?'🔴 HOOG':selectedResult.data.totLP>15?'🟡 MIDDEL':'🟢 LAAG'} · {year}
            </div>
            {[
              ['Laadpunten nodig', selectedResult.data.totLP],
              ['AC palen', selectedResult.data.totAC+selectedResult.data.totACS],
              ['DC palen', selectedResult.data.totDC],
              ['OV hubs',  selectedResult.data.totDCU],
              ['MWh/jr',   Math.round(selectedResult.data.totMwh)],
              ['CAPEX',    fmtEur(selectedResult.data.totCapex)],
            ].map(([l,v])=>(
        ...
        <div style={st.legB}>
          {[
            [{width:14,height:14,borderRadius:'50%',background:'#2ecc71',border:'2.5px solid #1a8a3a',boxShadow:'0 0 0 1.5px white'},'Bestaande AC paal'],
            [{width:14,height:14,borderRadius:'50%',background:'#e74c3c',border:'2.5px solid #c0392b',boxShadow:'0 0 0 1.5px white'},'Bestaande DC paal'],
            [{width:14,height:14,borderRadius:'50%',background:'#9b59b6',border:'2.5px solid #8e44ad',boxShadow:'0 0 0 1.5px white'},'Bestaande HPC paal'],
            [{width:10,height:10,borderRadius:'50%',background:'#9EC5CB44',border:'2px solid #9EC5CB'},'AC geprojecteerd'],
            [{width:10,height:10,borderRadius:'50%',background:'#D0AC4144',border:'2px solid #D0AC41'},'DC geprojecteerd'],
            [{width:10,height:10,borderRadius:'50%',background:'#c89ecb44',border:'2px solid #c89ecb'},'OV / Ultrafast'],
            [{width:14,height:10,borderRadius:2,background:'#2B5F6E44'},'Lage behoefte (<15)'],
            [{width:14,height:10,borderRadius:2,background:'#D0AC4166'},'Middel (15–30)'],
            [{width:14,height:10,borderRadius:2,background:'#E8683A66'},'Hoog (>30 LP)'],
          ].map(([ds,l],i)=>(
