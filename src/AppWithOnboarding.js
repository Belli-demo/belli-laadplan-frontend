...
function wijkFillColor(lp) {
  if (lp === 0)  return '#7BB8C4'; // blauw = geen extra nodig
  if (lp <= 5)   return '#A8D96B'; // groen = weinig nodig
  if (lp <= 15)  return '#F0C030'; // geel = beperkt nodig
  if (lp <= 30)  return '#2E9E55'; // donkergroen = extra nodig
  return '#D94030';                // rood = veel nodig
}
...
export default function AppWithOnboarding() {
  ...
  const loadBestaandePalen = useCallback(async () => {
    ...
    const q = `[out:json][timeout:20];(node["amenity"="charging_station"](${s},${w},${n},${e});way["amenity"="charging_station"](${s},${w},${n},${e}););out center;`;
    const endpoints = [
    ...
    for (const endpoint of endpoints) {
      ...
      try {
        ...
        geladen = true;
        console.log(`Overpass (${endpoint.includes('kumi') ? 'kumi' : 'main'}): ${json.elements?.length} laadpalen`);
      } catch(e) {
        console.warn(`Overpass endpoint fout (${endpoint}):`, e.message);
      }
    }
    if (!geladen) setExistingPalen([]);
