...
export default function AppWithOnboarding() {
  ...
  const totBestaand = wijkResults.reduce((s,r)=>s+r.data.bestaand,0);
  ...
  const tijdreeks = YEARS.map(yr => {
    ...
    const totBestaandJaar = res.reduce((s,r)=>s+r.bestaand,0);
    const totCapexJaar    = res.reduce((s,r)=>s+r.totCapex,0);
  ...
  useEffect(() => {
    ...
    existingPalen.forEach(el => {
      ...
      if(!lat) return;
      const t=el.tags||{};
      // Bepaal type: gebruik de officiële MOW-classificatie als die er is
      // (betrouwbaarder dan gokken op basis van OSM-tags), anders de oude
      // heuristiek als vangnet.
      ...
      let paalType;
      if (t.mow_snelheid === 'ultrasnel')      paalType = 'hpc';
      else if (t.mow_snelheid === 'snel')      paalType = 'dc';
      else if (t.mow_snelheid === 'normaal')   paalType = 'ac';
      else {
      ...
      L.marker([lat,lng],{icon:makeBestaandIcon(paalType), zIndexOffset: 1000})
       .bindPopup(`<b>Bestaande laadpaal</b><br>
         Type: ${paalType.toUpperCase()}${t.mow_toegankelijkheid ? ' · '+t.mow_toegankelijkheid : ''}<br>
         ${t.name||t.operator||'–'}<br>
         Netwerk: ${t.network||t.operator||'–'}<br>
         Vermogen: ${(t.maxpower||maxPower) ? (t.maxpower||maxPower)+'kW' : 'onbekend'}<br>
         ${t.mow_adres ? t.mow_adres+'<br>' : ''}
         <small>Bron: ${t.mow_snelheid ? 'MOW (Departement Mobiliteit en Openbare Werken)' : 'OpenStreetMap'}</small>`)
       .addTo(g);
    });
