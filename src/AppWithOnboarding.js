...
export default function AppWithOnboarding() {
  ...
  const loadBestaandePalen = useCallback(async () => {
    ...
    try {
      ...
      console.log('Overpass:', json.elements?.length, 'laadpalen gevonden');
    } catch(e) {
  ...
  useEffect(() => {
    ...
    if (sectoren?.features?.length > 0) {
      ...
      // Gebruik dichtheidsgewicht per sector (oppervlakte-inversie uit Statbel)
      const totaalLP  = wijkResults.reduce((s,r) => s + r.data.totLP, 0);
      const totaalMwh = wijkResults.reduce((s,r) => s + r.data.totMwh, 0);
      
      // Gewichten normaliseren zodat ze optellen tot 1
      const rawGewichten = sectoren.features.map(f => parseFloat(f.properties?.GEWICHT) || 1);
      const somGewicht   = rawGewichten.reduce((s,g) => s+g, 0);
      const normGewichten = rawGewichten.map(g => g / somGewicht);

      sectoren.features.forEach((feature, idx) => {
        const naam = feature.properties?.NAAM || `Sector ${idx+1}`;

        // LP op basis van dichtheidsgewicht
        // MWh afgeleid uit LP (consistente verhouding)
        const gewicht = normGewichten[idx];
        const lp  = Math.round(totaalLP  * gewicht);
        const mwh = totaalLP > 0 ? Math.round(totaalMwh * (lp / totaalLP)) : 0;
    ...
    wijkResults.forEach(({ wijk, data }, wijkIdx) => {
      ...
      if(totAC>0) addP(totAC,'#f39c12',pop('Geprojecteerd AC','7–22 kW'));
      if(totDC>0) addP(totDC,'#e74c3c',pop('Geprojecteerd DC Snel','50–150 kW'));
      if(totOV>0) addP(totOV,'#8e44ad',pop('Geprojecteerd DC Ultrafast / OV Hub','>150 kW'));
    });
      ...
      <div style={st.mArea}>
        ...
        <div style={st.legB}>
          {[
            ...
            [{width:12,height:12,borderRadius:'50%',background:'#f39c12',border:'2px solid white',boxShadow:'0 0 0 1px #f39c12'},'Geprojecteerd AC (7–22 kW)'],
            [{width:12,height:12,borderRadius:'50%',background:'#e74c3c',border:'2px solid white',boxShadow:'0 0 0 1px #e74c3c'},'Geprojecteerd DC (50–150 kW)'],
            [{width:12,height:12,borderRadius:'50%',background:'#8e44ad',border:'2px solid white',boxShadow:'0 0 0 1px #8e44ad'},'Geprojecteerd HPC / OV (>150 kW)'],
