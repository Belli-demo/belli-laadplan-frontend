...
import { getAlleGemeenten, getGemeente, slaGemeenteOp, verwijderGemeente, checkHealth } from './api';
...
export default function AppWithOnboarding() {
  ...
  useEffect(() => {
    ...
    wijkResults.forEach(({ wijk, data }) => {
      ...
      L.circle([wijk.lat, wijk.lng], {
        radius:300+data.totLP*18, color:fc, weight:2, fillColor:fc, fillOpacity:0.18,
      })
  ...
  }, [gemId, scenario, year, pubPct, activeSegs, trendFactor, gemeenten]);
          ...
          <div style={st.sec}>
            ...
            <div style={st.sBody}>
              ...
              {!STANDAARD_IDS.includes(gemId) && (
                <div style={{ fontSize:10, color:C.warn, cursor:'pointer', marginTop:4 }}
                  onClick={() => setShowDelete(true)}>
                  ✕ {gemeente?.naam} verwijderen
                </div>
              )}
