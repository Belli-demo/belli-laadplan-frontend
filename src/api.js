...
const API_URL = process.env.REACT_APP_API_URL || 'https://strategisch-laadplan-backend-production.up.railway.app';
...
export async function syncWijkenVanStatbel(id) {
  return apiFetch(`/geo/sync-wijken/${id}`, { method: 'POST' });
}

export async function getLaadpalen(id) {
  return apiFetch(`/geo/laadpalen/${id}`);
}
